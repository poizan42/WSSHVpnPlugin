...
- **Worker-thread injection without the doorbell does not work.** The M0'(5) probe
  ...
  worker) appends without error and the packet is never delivered. Provenance was challenged once
  (much of M0 predates the IPv6-address-list discovery, and a channel whose `Start` was rejected
  would swallow injections silently) and then settled from the preserved session logs: the probe
  ran repeatedly across 2026-08-14→16 on *healthy* channels — `StartWithMainTransport accepted
  (ipv6 1)`, and on the loopback builds the doorbell path's ICMP echo landed milliseconds before
  each silent worker append. The doorbell is load-bearing.
